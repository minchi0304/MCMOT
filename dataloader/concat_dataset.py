...
class ConcatDataset(torch.utils.data.ConcatDataset):
    def __init__(self, *datasets):
        ...
        for i,d in enumerate(self.datasets):
            ...
            upsample_shape = list(map(lambda x: int(x / d.img_reduce), [720, 1280]))
            ...
            self.config_dict['proj_mats'] = torch.stack([
                torch.from_numpy(map_zoom_mat @ imgcoord2worldgrid_matrices[cam] @ img_zoom_mat)
                for cam in range(d.num_cam)], dim=0) #벡터화
